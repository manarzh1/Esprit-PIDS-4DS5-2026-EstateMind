...
def _h_chat(body: dict) -> tuple[int, dict]:
    """POST /chat — chatbot juridique avec résumé intelligent et RAG optimisé"""
    question = body.get("question","").strip()
    if not question:
        return 400, {"error": "Champ requis : question"}

    # Paramètre de concision : "short" | "medium" | "detailed"
    sumup_level = body.get("sumup_level", None)
    ...
    if body.get("use_rag", True):
        try:
            ...
            res = search(question, sources=["CDR","URBANISME"], top_k=RAG_TOP_K)
            if res:
                rag_ctx = format_context(res)
        except Exception as e:
            log.warning(f"RAG search failed: {e}")

    result = chatbot_chat(question, rag_context=rag_ctx, sumup_level=sumup_level)
    return 200, result
