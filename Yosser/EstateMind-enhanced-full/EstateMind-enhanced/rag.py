...
from config  import LOG_DIR, TXT_CLEAN, RAG_TOP_K, RAG_MIN_SCORE, RAG_MAX_CONTEXT_CHARS, RAG_RERANK_ENABLED, RAG_DEDUP_THRESHOLD
...
def _rerank_chunks(query: str, results: list[dict]) -> list[dict]:
    """
    Reranking léger des chunks : booste les chunks dont le texte contient
    les mots-clés exacts de la query (en plus du score sémantique).
    ...
    for r in results:
        ...
        # Petit bonus de 0-15% selon le nombre de mots-clés présents
...
def _dedup_chunks(results: list[dict], threshold: float = None) -> list[dict]:
    """
    Supprime les chunks quasi-identiques dans les résultats RAG.
    Conserve le chunk avec le meilleur score.
    """
    thr = threshold or RAG_DEDUP_THRESHOLD
    kept = []
    for candidate in results:
        is_dup = False
        ctext = candidate["text"].lower()
        for existing in kept:
            etext = existing["text"].lower()
            # Similarité Jaccard rapide sur les n-grams
            cwords = set(re.findall(r"\w{4,}", ctext))
            ewords = set(re.findall(r"\w{4,}", etext))
            if cwords and ewords:
                sim = len(cwords & ewords) / len(cwords | ewords)
                if sim >= thr:
                    is_dup = True
                    break
        if not is_dup:
...
def search(
    ...
    Inclut reranking, déduplication et filtrage par score minimal.

    Args:
        query     : question de l'utilisateur
        sources   : ["CDR", "URBANISME"] ou None pour toutes
        top_k     : nombre de résultats (défaut : config RAG_TOP_K)
        min_score : seuil minimal de pertinence (défaut : config RAG_MIN_SCORE)

    Returns:
        Liste de chunks pertinents triés par score, dédupliqués.
    """
    top_k     = top_k     or RAG_TOP_K
    min_score = min_score or RAG_MIN_SCORE
    ...
    for src_key in sources:
        ...
        # Top-k filtré par seuil minimal
        indexed = sorted(enumerate(scores), key=lambda x: -x[1])[:top_k * 2]
    ...
    # Reranking
    all_results = _rerank_chunks(query, all_results)

    # Déduplication des chunks similaires
    all_results = _dedup_chunks(all_results)

    return all_results[:top_k]


def _score_to_confidence(score: float) -> str:
    """Convertit un score cosine en niveau de confiance lisible."""
    if score >= 0.70:
        return "élevée"
    elif score >= 0.45:
        return "moyenne"
...
def format_context(results: list[dict], max_chars: int = None) -> str:
    """
    Formate les résultats RAG en contexte structuré pour le LLM.
    Inclut le score de confiance pour chaque extrait.
    """
    if not results:
        return ""
    ...
    for r in results:
        ...
        conf_tag = f" [confiance: {confidence}]" if confidence else ""
        snippet = r["text"][:500]
        line = f"[{r['source_label']} — Art.{r['article']}{conf_tag}]\n{snippet}"
