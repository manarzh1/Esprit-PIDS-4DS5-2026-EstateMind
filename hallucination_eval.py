...
def find_semantic_duplicates(rules: list[dict], threshold: float = 0.85) -> list[tuple]:
    """
    Détecte les règles sémantiquement trop proches (doublons sémantiques).
    Retourne une liste de paires (i, j, similarité).
    """
    dups = []
    for i in range(len(rules)):
        for j in range(i + 1, len(rules)):
            ri, rj = rules[i], rules[j]
            if ri.get("actor") != rj.get("actor"):
                continue
            if ri.get("status") != rj.get("status"):
                continue
            sim = _sim(ri.get("action", ""), rj.get("action", ""))
            if sim >= threshold:
...
def evaluate(rules: list[dict]) -> dict:
    ...
    semantic_dups = find_semantic_duplicates(rules, threshold=0.85)
    dup_rules_idx = set()
