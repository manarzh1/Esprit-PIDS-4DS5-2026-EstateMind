...
RAG_MIN_SCORE          = 0.25   # seuil minimal de pertinence (cosine similarity)
RAG_MAX_CONTEXT_CHARS  = 2000   # taille maximale du contexte RAG
RAG_RERANK_ENABLED     = True   # activer le reranking des chunks
RAG_DEDUP_THRESHOLD    = 0.88   # seuil similarité pour déduplication chunks

# ─── Sum-Up (résumé automatique des réponses) ─────────────────────────────────
SUMUP_ENABLED          = True   # activer la fonction sum_up_response
SUMUP_DEFAULT_LEVEL    = "medium"  # "short" | "medium" | "detailed"
SUMUP_MAX_CHARS = {
    "short":    400,   # ~3-4 phrases
    "medium":   700,   # ~5-7 phrases
    "detailed": 1200,  # ~8-12 phrases
}
SUMUP_DEDUP_ENABLED    = True   # supprimer les phrases redondantes

# ─── Déduplication sémantique (évaluation) ────────────────────────────────────
SEMANTIC_DEDUP_THRESHOLD = 0.85   # seuil similarité pour doublons
SEMANTIC_DEDUP_ENABLED   = True   # activer la déduplication dans l'évaluation
